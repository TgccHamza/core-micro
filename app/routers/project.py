...
from . import file_transfer_pb2
from . import file_transfer_pb2_grpc
from .. import schemas
...
async def upload_file(module_id: str, file: UploadFile, db: Session = Depends(get_db)):
    ...
    with grpc.insecure_channel('grpc_url:50051') as channel:
        ...
        request = file_transfer_pb2.UploadRequest(filedata=file_data)
        response = stub.UploadFile(request)

    return services.set_template_module(db, module_id, response.code)
